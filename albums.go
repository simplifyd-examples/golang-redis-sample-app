...
type Album struct {
	...
	Artist string  `json:"artist" redis:"artist"`
...
func addAlbumToDB(album Album) (string, error) {
	id := uuid.New().String()
	conn := pool.Get()
	defer conn.Close()
	_, err := conn.Do("HMSET", "album:"+id, "title", album.Title, "artist", album.Artist, "price", album.Price, "likes", album.Likes)
	if err != nil {
		log.Println(err)
		return "", err
	}
